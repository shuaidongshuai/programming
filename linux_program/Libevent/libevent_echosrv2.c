...
* 直到libevent 通知我们可以写入，才能够真正的写入数据。这个简单的
buffer 队列用
* queue.h 中的TAILQ 来写入。
*/
struct bufferq {
...
*/
struct client {
	/* 事件对象。我们需要两个事件对象，一个用于读事件一个用于写事
	件通知。 */
	struct event ev_read;
	struct event ev_write;
	/* 这是数据队列，它将被写入这个客户端。我们只能等到libevent
	告诉我们这个socket 准备好写入时，
	* 才能真正写入。 */
...
/**
* 当客户端socket 可读时，libevent 调用这个函数。
*/
void
on_read(int fd, short ev, void *arg)
{
	...
	* 并且将它放到客户数据对象的写入队列。 */
	...
	/* 我们不能仅仅将buffer 写回，因为我们需要在可以写入时响应
	libevent 的可写事件，
	* 将缓冲区放到客户数据结构中的写队列，并安排一个可写事件。 */
	...
	/* 由于现在我们拥有需要写回到客户端的数据，所以添加一个可写事
	件通知。 */
	event_add(&client->ev_write, NULL);
}
/**
* 当客户端socket 准备好写入时，libevent 调用这个函数。
*/
void
on_write(int fd, short ev, void *arg)
{
	...
	struct bufferq *bufferq;
	int len;
	/* 将第一个元素移出写队列。我们可能不用再探测写队列是否是空，
	但是应该确认返回值不是NULL。 */
	bufferq = TAILQ_FIRST(&client->writeq);
	if (bufferq == NULL)
		return;
	/* 写buffer 里的数据。buffer 中的一部分可能已经在前面写入了，
	所以只写剩下的字节。 */
	...
	if (len == -1) {
		if (errno == EINTR || errno == EAGAIN) {
			/* 写操作被一个信号打断，或者我们不能写入数据，
			调整一下并返回。 */
			event_add(&client->ev_write, NULL);
			return;
		}
		else {
			/* 一些其它的socket 错误发生，退出吧。 */
			err(1, "write");
		}
	}
	else if ((bufferq->offset + len) < bufferq->len) {
		/* 不是所有的数据都被写入了，更新写入的偏移并调整写入
		事件。 */
		bufferq->offset += len;
		event_add(&client->ev_write, NULL);
		return;
...
/**
* 当有一个连接请求准备被接受时，libevent 将调用这个函数。
*/
void
on_accept(int fd, short ev, void *arg)
{
	...
	/* 我们已经接受了一个新的客户，分配一个客户数据结构对象来保存
	这个客户的状态。 */
	client = calloc(1, sizeof(*client));
	if (client == NULL)
		err(1, "malloc failed");
	/* 设置读事件，当客户端socket 可读时，libevent 将调用on_read()
	函数。
	* 我们也不断的构造可读事件，所以不用再每次读取时从新加入读事
	件了。 */
...
main(int argc, char **argv)
{
	...
	* code that I'll abstract away in the future. */
	...
	if (listen(listen_fd, 5) < 0)
		err(1, "listen failed");
	/* 设置socket 为非阻塞模式，这个在使用libevent 编程中时必须的。
	*/
	if (setnonblock(listen_fd) < 0)
		err(1, "failed to set server socket to non-blocking");
	/* 我们现在有了一个监听的socket，我们创建一个读事件，以便有客
	户连接请求时能够接收到通知。 */
