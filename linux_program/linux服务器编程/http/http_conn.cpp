...
http_conn::HTTP_CODE http_conn::process_read()
{
	...
	/*下面就是限状态机编程方法*/
    while ( ( ( m_check_state == CHECK_STATE_CONTENT ) && ( line_status == LINE_OK  ) )//主状态机：正在分析请求行。从状态机：读到完整行
                || ( ( line_status = parse_line() ) == LINE_OK ) )//从状态机必须读到\r\n
    {
        text = get_line();
...
http_conn::HTTP_CODE http_conn::do_request()
{
    ...
    m_file_address = ( char* )mmap( 0, m_file_stat.st_size, PROT_READ, MAP_PRIVATE, fd, 0 );//PROT_READ 页内容可以被读取
	printf("请求的文件大小：%d\n",m_file_stat.st_size);
    close( fd );
    return FILE_REQUEST;
}
...
bool http_conn::write()
{
    ...
    while( 1 )
    {
		...
		/*这里还有点不懂，这样做应该是说只要发出去一半我就不需要重复发了吧*/
        //if ( bytes_to_send <= bytes_have_send )
		if(m_write_idx <= bytes_have_send)
        {
            unmap();
            if( m_linger )
            {
