...
//        userService.delete(id);
    ...
    public String avatarEdit(Model model) {
        return "/user/avatarEdit";
    }

    @GetMapping("/avatarUpload")
    @ResponseBody
    public ResponseUploadDto avatarUpload() throws UnsupportedEncodingException {
        User user = authUserService.getCurUser();
        ResponseUploadDto responseUploadDto = ossService.uploadAvatar(user);
        return responseUploadDto;
    }

    @GetMapping("/avatarSave")
    @ResponseBody
    public String avatarSave(String fileUrl) {
        User user = authUserService.getCurUser();
