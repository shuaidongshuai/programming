...
public class Tank {
	...
	public Tank(int x,int y,boolean good,TankClient tc){//获取TankClient的引用，让Tank可以调用TankClient里面的数据，因为子弹的方向是由Tank决定的，而TankClient要获取Bullets方向
		this.x = x;
		this.y = y;
		this.good = good;
		this.tc = tc;
...
//		}
	...
	private Bullets fire(){//Tank发射子弹   给子弹一个信号，，让子弹往自己指定的方向发子弹
		...
		Bullets b = new Bullets(x,y,ptDir,tc);
		return b;
	}
