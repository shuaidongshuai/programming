...
public class TankClient extends Frame{
	...
	public int times1 = 0;//用来表示发了几发炮弹
	public List<Bullets> myBullets = new ArrayList<Bullets>();//代表还有几发炮弹
	public List<Bullets> myBullets_1 = new ArrayList<Bullets>();//代表还有几发炮弹
	public List<Explode> explodes = new ArrayList<Explode>();
	public List<Tank> enemyTank = new ArrayList<Tank>();//地方坦克
	public List<Tank> myTank = new ArrayList<Tank>();//地方坦克
	...
	public void paint(Graphics g) {
		...
		g.drawString("还有多少发子弹:"+(20-times), 10, 50);
		g.drawString("加农炮："+(20-times1), 10, 70);
		g.drawString("敌方坦克："+enemyTank.size(), 10, 90);
		
		g.setFont(new Font("", Font.BOLD, 25));//设置字体
		g.setColor(Color.red);
		if(times1>=20)
			g.drawString("还想要加农炮么？(充钱)", WIDTH/3, 60);
		else if(times>=20)
			g.drawString("没子弹了(充钱)", WIDTH/3, 60);
		
		for(int i=0;i<myBullets.size();i++){
			Bullets b = myBullets.get(i);//创建这一发炮弹	这一发子弹里面有坐标有方向
			b.hitTanks(enemyTank);//判断是否击中坦克（击中了就设置live标志为false）
			b.hitTank(myTank.get(0));
			b.hitWall(wall1);//设置子弹撞墙时的动作
			b.hitWall(wall2);//设置子弹撞墙时的动作
			b.draw(g);//当击中坦克，子弹自己就不再画自己了（通过坦克自己内部的live标志）
		}
		for(int i = 0;i<myBullets_1.size();i++){
			Bullets b = myBullets_1.get(i);
...
//		getGraphicsConfiguration().getDevice().setFullScreenWindow(this);
		...
		myTank.add(new Tank(50,500,true,Direction.STOP,this));//把TankClient对象传进去，使Tank可以操作TankClient里面的部分数据
		...
		this.addKeyListener(myTank.get(0).new MyKeyEvent());
