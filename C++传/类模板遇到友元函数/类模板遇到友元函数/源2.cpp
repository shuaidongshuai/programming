...
void add(A<int> &a, int i)	//我有了一个解释：当我注释掉a.age 之后就可以不分前后了
{							// 猜测：知道形参不被调用，那么形参就不会被编译，这样的话，就不会提前创建一个A<int>类出来，导致不能识别+这个符号
	/*a.age += i;			//为什么我这么猜呢？因为add一定会被先加载，，它不需要二次加载，而，operator+是需要二次加载的
	a.show();*/
	//cout << "hello" << endl;
}
