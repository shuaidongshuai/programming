...
int main( int argc, char* argv[] )
{
	...
	int l_onoff;
	int l_linger;
	};为了允许SO_LINGER，应用程序应将l_onoff设为非零，将l_linger设为零或需要的超时值（以秒为单位）
	...
	*/
    ...
    ret = listen( listenfd, 1000);//已完成队列100
    assert( ret >= 0 );
        ...
        for ( int i = 0; i < number; i++ )
        {
			...
			if( events[i].events & EPOLLIN )	
            {
				if( sockfd == listenfd )
				{
					...
					{
						/* 
						经过我几次验证，发现存在没有accept的socket 但是客户端是已经完成3次握手了的
						所以我每次进来都接收所有的socket
						*/
						...
						users[connfd].init( connfd, client_address );//初始化用户http对象
						//printf("connfd : %d--------------------------\n",connfd);
					}
				}
				else
            ...
            {
                pool->append( users + sockfd , false);//写任务
            }
        }
    }
