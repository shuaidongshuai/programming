...
public:
    ...
    static processpool< T > *create( int listenfd, int process_number = 50 )
    {
        if( !m_instance )
        {
...
private:
    //进程允许的最大子进程数量
    static const int MAX_PROCESS_NUMBER = 50;
    //每个子进程最多能处理的客户数量
...
int main( int argc, char* argv[] )
{
    ...
    processpool<D>* pool = processpool<D>::create( listenfd );
    if( pool )
    {
        pool->run();
        delete pool;
